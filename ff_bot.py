...
BANNER_API_URL = "https://image.killersharmabot.online/banner-image?headPic={headPic}&bannerId={bannerId}&name={name}&level={level}&guild={guild}&pinId={pinId}"
...
⏰ {stylish('Time Online:')} <code>{info.get('time_online', 'ɴ/ᴀ')}</code>
        ...
        # 👇 FIX: Only add parameters that have values
        banner_params = f"headPic={headpic}&bannerId={banner_id}"
        if name: banner_params += f"&name={name}"
        if level: banner_params += f"&level={level}"
        if guild: banner_params += f"&guild={guild}"
        if pin_id: banner_params += f"&pinId={pin_id}"
        
        if headpic and banner_id:
            banner_url = f"https://image.killersharmabot.online/banner-image?{banner_params}"
